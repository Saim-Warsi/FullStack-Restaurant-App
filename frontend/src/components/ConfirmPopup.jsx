    ...
    <div className="bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden transform transition-all duration-300 ease-out scale-100">
      ...
      <div className="px-6 py-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
            popupData.includes('delete') || popupData.includes('remove') || popupData.includes('cancel') 
              ? 'bg-red-100' 
              : 'bg-blue-100'
          }`}>
            {popupData.includes('delete') || popupData.includes('remove') || popupData.includes('cancel') ? (
          ...
          <h3 className="text-lg font-semibold text-gray-900">
            {popupData.includes('delete') || popupData.includes('remove') ? 'Confirm Removal' :
             popupData.includes('cancel') ? 'Confirm Cancellation' :
             popupData.includes('add') ? 'Confirm Addition' :
             'Confirm Action'}
          </h3>
        </div>
      </div>
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
        <div className="flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
          ...
          >
            {popupData.includes('delete') || popupData.includes('remove') ? 'Remove' :
             popupData.includes('cancel') ? 'Cancel Order' :
             popupData.includes('add') ? 'Add Item' :
             'Confirm'}
          </button>
        </div>
      </div>
